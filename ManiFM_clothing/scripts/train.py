...
def custom_loss(pred_force, true_force, pred_force_point, true_force_point, posterior_prob):
    force_loss = nn.MSELoss()(pred_force, true_force)
    force_point_loss = nn.MSELoss()(pred_force_point, true_force_point)
    posterior_loss = -torch.log(posterior_prob + 1e-8)  
    total_loss = force_loss + force_point_loss + posterior_loss
    return total_loss
...
def train():
    ...
    for epoch in progress_bar:
        ...
        if epoch % cfg.train.save_epoch == 0:
            ...
            accelerator.save_state(os.path.join(cfg.dir.model_dir, f"model_{epoch}"))
