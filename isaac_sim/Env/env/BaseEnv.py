...
class BaseEnv:
    def __init__(self,scene_config:SceneConfig=None,rigid:bool=False,deformable:bool=False,garment:bool=False):
        ...
        prim = self.stage.GetPrimAtPath("/World/Room/room/Camera")
        vpi = omni.kit.viewport.utility.get_active_viewport()
        vpi.set_active_camera(str(prim.GetPath()))
    ...
    def import_room(self,scene_config:SceneConfig):
        ...
        self.room_name=self.room_prim_path.split("/")[-1] #find_unique_string_name(initial_name="room",is_unique_fn=lambda x: not self.world.scene.object_exists(x))
