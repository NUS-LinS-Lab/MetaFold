...
from omni.isaac.core.objects import DynamicSphere,DynamicCuboid,FixedCuboid,VisualSphere,VisualCylinder,VisualCone
...
class AttachmentBlock():
    def __init__(self,world:World, robot:MyFranka, init_place, collision_group=None, no = 0):
        self.world=world
        ...
        self.cube_name=self.attachment_path.split('/')[-1]#find_unique_string_name(initial_name="cube", is_unique_fn=lambda x: not self.world.scene.object_exists(x))
        self.collision_group=collision_group
        self.block_control=self.create(no)
   


        

    def create(self,no = 0):
        if no!=0:
            if no==1:
                prim = VisualSphere(prim_path=self.block_path, color=np.array([1.0, 1.0, 1.0]),
                        name=self.cube_name,
                ...
                if no==3:
                    prim = VisualCylinder(prim_path=self.block_path, color=np.array([1.0, 1.0, 1.0]),
                            name=self.cube_name,
                            position=self.init_place,
                            scale=np.array([0.08, 0.08, 0.08]),
                            visible=True,
        ...
        prim = DynamicSphere(prim_path=self.block_path, color=np.array([1.0, 1.0, 1.0]),
                    name=self.cube_name,
                    position=self.init_place,
    ...
    def get_velocities(self):
        ...
        velocities=self.move_block_controller.get_velocities()
        return velocities
